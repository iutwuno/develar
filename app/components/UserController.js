...
import {UserCol, UserManager} from "./User";
import UserView from "./UserView";
import UserEdit from "./UserEdit";
import UserViewLayout from "./UserViewLayout";
import UserViewCol from "./UserViewCol";
import userEditForm from "../templates/userEditForm.jst";
...
var fetchUsersFromDB = function(cb){
	var users = [
		{
			username: "jlorenzo1111",
			displayName:"Jorge Lorenzo2222",
			mail: "jlorenzo@hotmail.com3333",
		},
		{
			displayName:"alfredo",
...
var editUserForm = function(user){
	...
	form.on("blur", function(form, editor){
		...
		if(error){
			...
			UserManager.updateUser(form.model, userCollection);
...
var renderUserFormModal = function(form){
	...
	modal.on("ok",function(){
		...
		if(!errors){
			...
			UserManager.addUser(form.model, userCollection);
